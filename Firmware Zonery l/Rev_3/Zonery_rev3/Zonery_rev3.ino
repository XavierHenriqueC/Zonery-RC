...
const int batteryPin = 2; // Pino de entrada analógica
const float maxVoltage = 3.3; // Tensão máxima de referência (volts)
...
void readBatteryVoltage() {
  // Leitura da tensão da bateria
  int rawValueBattery = analogRead(batteryPin);
  // Converte o valor lido para uma escala de 0 a 100%
  float percentageBattery = (rawValueBattery / 4095.0) * 100.0;
  ...
  if (percentageBattery >= 75.0) {
    // Verde
    setLed_R = 0;
    setLed_G = 254;
    setLed_B = 0;
  } else if (percentageBattery >= 50.0 && percentageBattery < 75.0) {
    // Amarelo
    setLed_R = 254;
    setLed_G = 254;
    setLed_B = 0;
  } else if (percentageBattery >= 25.0 && percentageBattery < 50.0) {
    // Laranja
    setLed_R = 254;
    setLed_G = 192;
    setLed_B = 0;
...
void setup() {
  ...
  //Printa o MAC Address do Bluetoth da ESP32
